 ...
 *
 * 키 문자열을 훅마다 직접 박으면 조회 쪽과 무효화 쪽이 조용히 어긋난다.
 * (실제로 목록은 `['posts', id]`, 상세는 `['post', id]`로 갈려 있어서 목록만 무효화되고
 * 상세는 낡은 값을 유지하고 있었다) 그래서 키는 전부 여기서 만든다.
 *
 * ## 규칙
 *
 * 1. 도메인마다 `all`을 두고, **도메인 전체를 무효화할 때만** 쓴다.
 * 2. 세부 키는 `[...all, '역할', 파라미터]` 형태로 `all`을 접두사로 갖는다.
 *    접두사가 겹치므로 `all` 무효화가 하위 키까지 자동으로 덮는다.
 * 3. 새 도메인을 연동할 때 여기에 먼저 키를 추가하고 훅에서 가져다 쓴다.
 *    (훅에 문자열 배열을 직접 쓰지 않는다)
 */
